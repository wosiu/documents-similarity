...
public class BucketsCreator extends EvalFunc<DataBag> {
	...
	public List<List<String>> createDocBuckets(List<Band> bands) {
		Map<List<Long>, List<String>> mapping = createMapping(bands);
		List<List<String>> result = new ArrayList<List<String>>(mapping.values());
		return  result;
	}
	...
	public DataBag exec(Tuple tuple) throws IOException {
		...
		while(it.hasNext()) {
			...
			bands.add(new Band(0, bandSignature, docName));
		}
